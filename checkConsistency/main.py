...
def read_template_from_file():
    try:
        print("Reading test_harness.c file...")
        with open("test_harness.c", 'r') as file:
            file_content = file.read()
            print(f"File content: {file_content}")
            return file_content
    except FileNotFoundError:
        print("Error: test_harness.c file not found. Using default template.")
        return ""

C_TEMPLATE = read_template_from_file()
...
def check_equivalence(file_a, file_b, timeout=300, output_dir=None, docker_image="klee/klee:3.0"):
    ...
    try:
        ...
        if "completed paths = " in klee_result.stderr:
            completed_str = klee_result.stderr.split("completed paths = ")[1].split("\n")[0].strip()
            try:
                completed_paths = int(completed_str)
            except:
                pass
                
        if "partially completed paths = " in klee_result.stderr:
            partial_str = klee_result.stderr.split("partially completed paths = ")[1].split("\n")[0].strip()
            try:
